...
static void Cmd_attackcanceler(void)
{
    ...
    else if (gProtectStructs[gBattlerTarget].nonSequiturCharge && IsMoveMakingContact(gCurrentMove, gBattlerAttacker))
    {
        gProtectStructs[gBattlerAttacker].touchedProtectLike = TRUE;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    if (gCurrentMove == MOVE_ILL_FATE && gBattleMons[gBattlerTarget].status1 & STATUS1_ANY_NEGATIVE)
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_SLIP_N_WHIP:
        if (IsMoveMakingContact(move, battlerAtk))
            calc = (calc * 80) / 100; // 1.2 slip n whip loss
        break;
    ...
    if (!gSpecialStatuses[gBattlerAttacker].ppNotAffectedByPressure)
    {
        ...
        case MOVE_TARGET_FOES_AND_ALLY:
            for (i = 0; i < gBattlersCount; i++)
            {
                ...
                if ((gBattleMoves[gCurrentMove].type == TYPE_POISON
                || gBattleMoves[gCurrentMove].type == TYPE_GHOST)
                && GetBattlerAbility(i) == ABILITY_MISFORTUNE)
                {
                    ppToDeduct += 1;
                }
                else 
                ...
                {
                    ppToDeduct *= 2;
                    if (gBattleMons[gBattlerAttacker].pp[gCurrentMove] < ppToDeduct)
                        ppToDeduct = gBattleMons[gBattlerAttacker].pp[gCurrentMove];
                }

                if ((GetBattlerAbility(gBattlerAttacker) == ABILITY_RAPID_FIRE 
                || GetBattlerAbility(gBattlerAttacker) == ABILITY_DARING_DEED)
        ...
        case MOVE_TARGET_OPPONENTS_FIELD:
            for (i = 0; i < gBattlersCount; i++)
            {
                ...
                else 
                {
                    ...
                    ppToDeduct += ((gBattleMoves[gCurrentMove].type == TYPE_POISON || gBattleMoves[gCurrentMove].type == TYPE_GHOST) && GetBattlerAbility(gBattlerTarget) == ABILITY_MISFORTUNE);
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    || abilityDef == ABILITY_SHELL_ARMOR || abilityDef == ABILITY_IGNORANT_BLISS)
    {
        critChance = -1;
             ...
             || (gCurrentMove == MOVE_GREASE_LIGHTNING && gDisableStructs[battlerDef].tarShot)
             ...
             || (gCurrentMove == MOVE_ILL_FATE && gBattleMons[battlerDef].status1 & STATUS1_ANY_NEGATIVE)
    ...
    else
    {
        critChance  = 2 * ((gBattleMons[battlerAtk].status2 & STATUS2_FOCUS_ENERGY) != 0)
                    ...
                    + 2 * (abilityAtk == ABILITY_DARING_DEED)
...
void SetMoveEffect(bool32 primary, u32 certain)
{
    ...
    case MOVE_EFFECT_STOCKPILE_AWAY:
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            if (sStatusFlagsForMoveEffects[gBattleScripting.moveEffect] == STATUS1_SLEEP)
                ...
                gBattleMons[gEffectBattler].status1 |= STATUS1_REST_TURN(1 + RandomUniform(RNG_SLEEP_TURNS, 1, 3));
            ...
            if (battlerAbility == ABILITY_HEAT_SEEKER)
            {
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_SPATK_SPDEF_UP:
                ...
                break;
            case MOVE_EFFECT_ATK_SPDEF_UP:
                if (!NoAliveMonsForEitherParty())
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_AtkSpDefUp;
                }
                break;
            ...
            case MOVE_EFFECT_THROAT_CHOP:
                ...
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_TAR:
                if (!NoAliveMonsForEitherParty())
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_MoveEffectTarShot;
                }
                break;
            case MOVE_EFFECT_FRESH_WHIP:
                if ((!(NoAliveMonsForEitherParty())) && (!(gStatuses3[gBattlerAttacker] & STATUS3_HEAL_BLOCK)))
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_MoveEffectFreshWhip;
                }
                break;
            ...
            case MOVE_EFFECT_TRIPLE_ARROWS:
                ...
                break;
            case MOVE_EFFECT_IRON_JAWS:
                {
                    u8 randomLowerDefenseChance = RandomPercentage(RNG_TRIPLE_ARROWS_DEFENSE_DOWN, CalcSecondaryEffectChance(gBattlerAttacker, 100));
                    u8 randomFlinchChance = RandomPercentage(RNG_TRIPLE_ARROWS_FLINCH, CalcSecondaryEffectChance(gBattlerAttacker, 30));
    ...
    do
    {
        ...
        case MOVEEND_PROTECT_LIKE_EFFECT:
            if (gProtectStructs[gBattlerAttacker].touchedProtectLike)
            {
                ...
                else if (gProtectStructs[gBattlerTarget].nonSequiturCharge
                         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
                {
                    i = Random() % 4;
                    ...
                    else if (i == 1 && CanGetFrostbite(gBattlerAttacker))
                    {
                    ...
                    else if (i == 3 && CanBeParalyzed(gBattlerAttacker))
                    {
                        ...
                        MarkBattlerForControllerExec(gBattlerAttacker);
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_NonSequiturParalyze;
                        effect = 1;
                    }
                    else
                    {
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_NonSequiturRet;
                    }
                }
        ...
        case MOVEEND_RECOIL:
                ...
                && gBattleScripting.savedDmg != 0) // Some checks may be redundant alongside this one
            {
                ...
                case EFFECT_ICEBREAKER: // Icebreaker, may OHKO
        ...
        case MOVEEND_MOVE_EFFECTS2: // For effects which should happen after target items, for example Knock Off after damage from Rocky Helmet.
        {
            ...
            case MOVE_EFFECT_STOCKPILE_AWAY:
                if (gDisableStructs[gBattlerAttacker].stockpileCounter != 0)
                {
                    ...
                    effect = TRUE;
                    BattleScriptPush(gBattlescriptCurrInstr);
                    gBattlescriptCurrInstr = BattleScript_MoveEffectStockpileAway;
                }
                break;
            case MOVE_EFFECT_SMACK_DOWN:
        ...
        case MOVEEND_MULTIHIT_MOVE:
        {
            ...
            && !(gCurrentMove == MOVE_PRESENT && gBattleStruct->presentBasePower == 0)) // Silly edge case
            {
                ...
                if (gMultiHitCounter == 0)
                {
                    ...
                    if (gBattleMoves[gCurrentMove].argument == MOVE_EFFECT_ASTRAL_SPIKES && !NoAliveMonsForEitherParty())
                    {
                        BattleScriptPush(gBattlescriptCurrInstr + 1);
        ...
        case MOVEEND_EJECT_BUTTON:
            if (gBattleMoves[gCurrentMove].effect != EFFECT_HIT_SWITCH_TARGET
              && gBattleMoves[gCurrentMove].effect != EFFECT_PLIA_BALL
        ...
        case MOVEEND_RED_CARD:
            if ((gBattleMoves[gCurrentMove].effect != EFFECT_HIT_SWITCH_TARGET || gBattleStruct->hitSwitchTargetFailed)
              && (gBattleMoves[gCurrentMove].effect != EFFECT_PLIA_BALL || gBattleStruct->hitSwitchTargetFailed)
        ...
        case MOVEEND_REBOUND: // Special case, because moves hitting multiple opponents stop after switching out
            for (i = 0; i < gBattlersCount; i++)
            {
                if (gBattleResources->flags->flags[i] & RESOURCE_FLAG_REBOUND)
                {
                    gBattleResources->flags->flags[i] &= ~RESOURCE_FLAG_REBOUND;
        ...
        case MOVEEND_CLEAR_BITS: // Clear/Set bits for things like using a move for all targets and all hits.
            ...
            if (!IS_MOVE_STATUS(gCurrentMove))
                gStatuses4[gBattlerAttacker] &= ~(STATUS4_FILTHMONGER);
...
static void Cmd_switchineffects(void)
{
    ...
    else if (((gBattleStruct->storedHealingWish & gBitTable[battler])
        || (gBattleStruct->storedLunarDance & gBitTable[battler]))
        && (gBattleMons[battler].hp != gBattleMons[battler].maxHP
        || gBattleMons[battler].status1 != 0
        || B_HEALING_WISH_SWITCH < GEN_8))
    {
...
static void Cmd_various(void)
{
   ...
   case VARIOUS_DESTINY_KNOT_TORMENT:
    ...
    case VARIOUS_TRY_ACTIVATE_VISCERATION:
    {
        VARIOUS_ARGS();
        while (gBattleStruct->viscerationBattlerId < gBattlersCount)
        {
            gBattleScripting.battler = gBattleStruct->viscerationBattlerId++;
            if (GetBattlerAbility(gBattleScripting.battler) == ABILITY_VISCERATION
                && IsBattlerAlive(gBattleScripting.battler)
                && !NoAliveMonsForEitherParty()
                ...
                && !(gStatuses3[gBattleScripting.battler] & STATUS3_HEAL_BLOCK))
            {
                ...
                gBattleMoveDamage *= -1;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_ScriptingAbilityHeal;
                return;
            }
        }
        gBattleStruct->viscerationBattlerId = 0;
        break;
    }
    ...
    case VARIOUS_JUMP_IF_HP_THRESHOLD:
    {
        ...
        else if (gCurrentMove == MOVE_ROAST_ROUST || gCurrentMove == MOVE_CHAKRA_SURGE)
        {
    ...
    case VARIOUS_TRY_HEAL_HALF_MISSING_HP:
    {
        VARIOUS_ARGS(const u8 *failInstr);
        gBattleMoveDamage = (gBattleMons[battler].maxHP - gBattleMons[battler].hp) / 2;
        if (gBattleMoveDamage == 0)
            gBattleMoveDamage = 1;
        gBattleMoveDamage *= -1;
    ...
    case VARIOUS_JUMP_IF_NO_STOCKPILE:
    {
        VARIOUS_ARGS(const u8 *jumpInstr);
        if (gDisableStructs[battler].stockpileCounter == 0)
        {
            gBattlescriptCurrInstr = cmd->jumpInstr;
        }
        else
    ...
    case VARIOUS_SET_SAND_SLIP:
    {
        VARIOUS_ARGS(const u8 *failInstr);
        if (gDisableStructs[battler].sandSlip)
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else
        {
            gDisableStructs[battler].sandSlip = TRUE;
            gBattleMons[battler].status2 |= STATUS2_ESCAPE_PREVENTION;
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        return;
    ...
    case VARIOUS_SET_NON_SEQUITUR:
    {
        VARIOUS_ARGS();
        gProtectStructs[battler].nonSequiturCharge = TRUE;
        break;
    }
    ...
    case VARIOUS_SET_FILTHMONGER:
    {
        VARIOUS_ARGS();

        u8 battler = GetBattlerForBattleScript(cmd->battler);
        gStatuses4[battler] |= STATUS4_FILTHMONGER;
        gDisableStructs[battler].filthmongerTimer = 0;
        gBattlescriptCurrInstr++;
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
...
static void Cmd_stockpiletobasedamage(void)
{
    ...
    else
    {
        ...
        {
            if (gCurrentMove == MOVE_NANAB_GATTLING && (gMultiHitCounter == 0 || gMultiHitCounter == 1))
            {
                gBattleStruct->moveEffect2 = MOVE_EFFECT_STOCKPILE_AWAY;
            }
            else
            {
...
static void Cmd_tryKO(void)
{
    ...
    {
        if (gCurrentMove == MOVE_ICEBREAKER && (gBattleMons[gBattlerTarget].status1 & STATUS1_FROSTBITE || gBattleMons[gBattlerTarget].status1 & STATUS1_FREEZE))
        {
            lands = TRUE;
        }
...
static void Cmd_tryworryseed(void)
{
    ...
    else
    {
        ...
        else if (gCurrentMove == MOVE_HEART_ATTACK)
        {
            gBattleMons[gBattlerTarget].ability = gBattleStruct->overwrittenAbilities[gBattlerTarget] = ABILITY_PALPITATIONS;
        }
        else
        {
