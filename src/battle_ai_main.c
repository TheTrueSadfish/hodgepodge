    ...
    // other
        ...
        case EFFECT_EMBER_SNOW:
            if (predictedMove != MOVE_NONE
             && aiData->abilities[battlerAtk] == ABILITY_HEAT_SEEKER
             && gBattleMons[battlerAtk].species == SPECIES_RICKSHAWTY_CHARIOT)
                score += 3;
        ...
        case EFFECT_FIREWORK_CRASH:
        case EFFECT_ICEBREAKER:
            if (aiData->abilities[battlerAtk] != (ABILITY_MAGIC_GUARD || ABILITY_SUGAR_COAT) && aiData->abilities[battlerAtk] != ABILITY_ROCK_HEAD)
            {
        ...
        case EFFECT_SAND_SLIP:
            if (gDisableStructs[battlerDef].sandSlip
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                score -= 10;
            else if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_DEF))
                score -= 10;
        ...
        case EFFECT_WORRY_SEED:
            if (!AI_CanPanic(battlerAtk, battlerDef, aiData->abilities[battlerDef], move, aiData->partnerMove))
              ...
              || aiData->holdEffects[battlerDef] == HOLD_EFFECT_ABILITY_SHIELD)
                score -= 6;
            break;
        case EFFECT_HEART_ATTACK:
            if (aiData->abilities[battlerDef] == ABILITY_PALPITATIONS
            || IsWorrySeedBannedAbility(aiData->abilities[battlerDef])
            || aiData->holdEffects[battlerDef] == HOLD_EFFECT_ABILITY_SHIELD)
                score -= 6;
            break;
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SOAK:
                ...
                break;
            case EFFECT_BELDAM_BREW:
                if (atkPartnerAbility == ABILITY_WONDER_GUARD
                 && (GetBattlerType(battlerAtkPartner, 0) != TYPE_POISON
                 || GetBattlerType(battlerAtkPartner, 1) != TYPE_POISON
                 || GetBattlerType(battlerAtkPartner, 2) != TYPE_POISON))
                {
                    RETURN_SCORE_PLUS(1);
                }
                break;
...
static s32 AI_CheckViability(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    case EFFECT_KELP_SAP:
        if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_BIG_ROOT)
            score++;
        if (aiData->abilities[battlerAtk] == ABILITY_HUDDLE_UP)
            score++;
...
// stat lowering effects
    ...
    case EFFECT_DRAG_DOWN:
        switch (ShouldPivot(battlerAtk, battlerDef, aiData->abilities[battlerDef], move, AI_THINKING_STRUCT->movesetIndex))
            ...
            case 0: // no
                ...
                break;
            case 1: // maybe
                score += min(CountPositiveStatStages(battlerDef), 2);
                break;
            case 2: // yes
                score += 5;
                score += min(CountPositiveStatStages(battlerDef), 2);
    ...
    case EFFECT_STOCKPILE:
        ...
        if (HasMoveEffect(battlerAtk, EFFECT_SWALLOW)
          ...
          || HasMoveEffect(battlerAtk, EFFECT_NANAB_GATTLING))
            score += 2;
    ...
    case EFFECT_WITCH_HYMN:
        ...
        {
            if (gBattleMons[battlerDef].statStages[i] < DEFAULT_STAT_STAGE)
                score += CountBattlerStatDecreases(battlerDef, TRUE) * 2;
        }
        break;
    case EFFECT_ROLLOUT:
    ...
    case EFFECT_DARK_HUNGER:
        if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_BIG_ROOT)
            score++;
        else if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE || aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD)
            break;
        else if (effectiveness <= AI_EFFECTIVENESS_x0_5 && AI_RandLessThan(50))
            score -= 3;
        else if (ItemId_GetPocket(aiData->items[battlerDef]) == POCKET_BERRIES)
            score += 3;
        break;
    ...
    case EFFECT_SOAK:
        ...
        break;
    case EFFECT_BELDAM_BREW:
        if (HasMoveWithType(battlerAtk, TYPE_PSYCHIC) || HasMoveWithType(battlerAtk, TYPE_GROUND))
            score += 2; // Get some super effective moves
        if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER && predictedMove != MOVE_NONE && IsHealingMoveEffect(gBattleMoves[predictedMove].effect))
            score += 3; // Try to cancel healing move
        else if (HasHealingEffect(battlerDef) || aiData->holdEffects[battlerDef] == HOLD_EFFECT_LEFTOVERS
    ...
    case EFFECT_SMELLINGSALT:
        if (gBattleMons[battlerDef].status1 & STATUS1_PARALYSIS)
            score += 2;
        break;
    case EFFECT_HAUNT_MELODY:
        if (gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY)
            score += 5;
        break;
    case EFFECT_ICEBREAKER:
        if (gBattleMons[battlerDef].status1 & (STATUS1_FROSTBITE | STATUS1_FREEZE))
            score += 10;
        break;
    case EFFECT_LOVELY_POISON:
        if ((gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY) & (gBattleMons[battlerDef].status2 & STATUS2_INFATUATION))
            break;
        if (gBattleMons[battlerDef].status2 & STATUS2_INFATUATION)
            score += 3;
        if (gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY)
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY || effect == EFFECT_FLORAL_HEALING)
         ...
         || (moveType == TYPE_FIRE && AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_HEAT_SEEKER)
