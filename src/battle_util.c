...
void PressurePPLose(u8 target, u8 attacker, u16 move)
{
    ...
    || targetAbility != ABILITY_MISFORTUNE
    ...
    {
        if ((gBattleMoves[moveIndex].type == TYPE_POISON
        || gBattleMoves[moveIndex].type == TYPE_GHOST)
        && targetAbility == ABILITY_MISFORTUNE)
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
                                          ...
                                          || (GetBattlerAbility(i) == ABILITY_SHUNYONG && gBattleResults.battleTurnCounter % 2 != 0)))
        {
            ...
            if (j != MAX_MON_MOVES)
            {
                ...
                {
                    if ((gBattleMoves[attacker].type == TYPE_POISON
                    || gBattleMoves[attacker].type == TYPE_GHOST)
                    && GetBattlerAbility(i) == ABILITY_MISFORTUNE)
                    {
                        gBattleMons[attacker].pp[j]--;
                    }
                    else if (GetBattlerAbility(i) == ABILITY_PRESSURE)
                    {
                        gBattleMons[attacker].pp[j]--;
                    }
...
void PrepareStringBattle(u16 stringId, u32 battler)
{
    ...
    if (stringId == STRINGID_STATSWONTDECREASE && !(gBattleScripting.statChanger & STAT_BUFF_NEGATIVE))
        ...
        stringId = STRINGID_STATSWONTDECREASE2;
              ...
              && battlerAbility == ABILITY_COVEN_LIGHTS 
              && BattlerCanHaveStatLowered(gBattlerTarget)
              && ((!(gProtectStructs[gBattlerTarget].covenLights1)) || (!(gProtectStructs[gBattlerTarget].covenLights2)))
              ...
              && gSideTimers[targetSide].stickyWebBattlerSide == targetSide)) // Sticky Web must have been set by the foe
    {
        if (gProtectStructs[gBattlerTarget].covenLights1)
            gProtectStructs[gBattlerTarget].covenLights2 = TRUE;
        else 
            gProtectStructs[gBattlerTarget].covenLights1 = TRUE;
        
        gBattleScripting.stickyWebStatDrop = 0;
        gBattlerAbility = gBattlerAttacker;
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_CovenLightsLowersDefenderStat;
...
s32 GetDrainedBigRootHp(u32 battler, s32 hp)
{
    if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_BIG_ROOT)
        hp = (hp * 1500) / 1000;
    if (GetBattlerAbility(battler) == ABILITY_HUDDLE_UP)
        hp = (hp * 2000) / 1000;
    if (hp == 0)
        hp = 1;
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_BURN: // burn
            if ((gBattleMons[battler].status1 & STATUS1_BURN) && gBattleMons[battler].hp != 0 && !((GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_TERU_CHARM) && (gBattleMons[battler].species == SPECIES_CHIROBERRA)))
            {
                ...
                {
                    if (gBattleMoveDamage < (gBattleMoveDamage * 2) + 1) // Record ability if the burn takes more damage than it normally would.
        ...
        case ENDTURN_SAND_SLIP:
            if (gDisableStructs[battler].sandSlip && gBattleMons[battler].hp != 0)
            {
                ...
                if (gBattleMoveDamage == 0)
                    gBattleMoveDamage = 1;
                PREPARE_MOVE_BUFFER(gBattleTextBuff1, MOVE_SAND_SLIP);
                BattleScriptExecute(BattleScript_SandSlipEndTurn);
                effect++;
            }
            gBattleStruct->turnEffectsTracker++;
            break;
        ...
        case ENDTURN_PUMPED_UP: // reservoir
            ...
            gBattleStruct->turnEffectsTracker++;
            break;
        case ENDTURN_FILTHMONGER: // filthmonger
            if (gDisableStructs[battler].filthmongerTimer > 0 && --gDisableStructs[battler].filthmongerTimer == 0)
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_ALL_GAME:
            ...
            break;
        case ABILITY_TERRAFORMER:
            if (!(gSpecialStatuses[battler].switchInAbilityDone))
            {
                u32 terraformer = Random() % 100;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                if (terraformer < 25)
                {
                    goto ELECTRIC;
                }
                else if (terraformer < 50)
                {
                    goto GRASSY;
                }
                else if (terraformer < 75)
                {
                    goto MISTY;
                }
                else
        ...
        case ABILITY_SURPRISE:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SurpriseActivates);
                effect++;
            }
            break;
        case ABILITY_TRACE:
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            case ABILITY_PANACEA:
                if (!BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && IsBattlerAlive(battler))
                {
                    gBattleMoveDamage = gBattleMons[battler].maxHP / 8;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
            ...
            case ABILITY_BAD_DREAMS:
                ...
                effect++;
                break;
            case ABILITY_SPACE_CROOK:
                BattleScriptPushCursorAndCallback(BattleScript_SpaceCrookActivates);
                effect++;
                break;
            case ABILITY_MIASMA:
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_MARSHLANDER:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && TARGET_TURN_DAMAGED && IsBattlerAlive(battler) && (moveType == TYPE_WATER || moveType == TYPE_GRASS) && CompareStat(battler, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN))
            {
                gEffectBattler = battler;
        ...
        case ABILITY_SLIP_N_WHIP:   
            ...
            && !(gBattleMons[gBattlerTarget].status1 & STATUS1_FREEZE))
            {
                ...
                if (gStatuses3[gBattlerAttacker] & STATUS3_HEAL_BLOCK)
                    ...
                    extraMoveSecondaryEffect = MOVE_EFFECT_FRESH_WHIP;  //Leave at 0 to remove it's secondary effect
                gTempMove = gCurrentMove;
                gCurrentMove = extraMove;
                gMultiHitCounter = 0;
                gProtectStructs[battler].extraMoveUsed = TRUE;
        ...
        case ABILITY_DISGUISE:
            ...
            && (gMultiHitCounter == 0 || gMultiHitCounter == 1)
            && IsBattlerAlive(gBattlerAttacker)
            && (!(gStatuses4[gBattlerAttacker] & STATUS4_INFECTION))
        ...
        case ABILITY_PAINTED_HAZARD:
            ...
            && moveType == TYPE_ROCK)
            {
                SWAP(gBattlerAttacker, gBattlerTarget, i);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_PaintedHazardStealthRock;
                effect++;
            }
            else if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
            && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
            ...
            && (gSideTimers[gBattlerAttacker].spikesAmount != 3))
            {
                SWAP(gBattlerAttacker, gBattlerTarget, i);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_PaintedHazardSpikes;
                effect++;
            }
            break;
        }
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_SUNK_COST:
            ...
            && !(IS_MOVE_STATUS(gCurrentMove)))
            {
                ...
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_SunkCostMessage;
                effect++;
            }
            break;
        case ABILITY_ICE_SCALES:
        ...
        case ABILITY_GOOSE_CHASER:
            ...
            && (gMultiHitCounter == 0 || gMultiHitCounter == 1))
            {
                ...
                if (gDisableStructs[gBattlerTarget].tarShot)
                    extraMoveSecondaryEffect = MOVE_EFFECT_SPD_MINUS_1;  //Leave at 0 to remove it's secondary effect
                else
                    extraMoveSecondaryEffect = MOVE_EFFECT_TAR;  //Leave at 0 to remove it's secondary effect
                gTempMove = gCurrentMove;
                gCurrentMove = extraMove;
                gMultiHitCounter = 0;
                gProtectStructs[battler].extraMoveUsed = TRUE;
        ...
        case ABILITY_TOOTH_N_NAIL:
            ...
            {    
                ...
                if (gBattleMoves[move].bitingMove || gBattleMoves[move].soundMove)
                {
                    ...
                    {
                        extraMove = MOVE_CRUSH_CLAW;  //The Extra Move to be used
                        movePower = 75;                  //The Move power, leave at 0 if you want it to be the same as the normal move
                        moveEffectPercentChance  = 50;  //The percent chance of the move effect happening
                ...
                else if (gBattleMoves[move].slicingMove || gCurrentMove == MOVE_FURY_SWIPES)
                {
                    ...
                    {
                        extraMove = MOVE_HOWL;  //The Extra Move to be used
...
u32 GetBattlerAbility(u32 battler)
{
    ...
    || gBattleMons[battler].ability == ABILITY_INSTABILITY
...
#define TERRAIN_TYPE_BOOST UQ_4_12(1.5)
    ...
    if (gStatuses4[battlerAtk] & STATUS4_PUMPED_UP && moveType == TYPE_WATER)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
    if (gStatuses4[battlerAtk] & STATUS4_FILTHMONGER)
        modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
...
static inline uq4_12_t GetAttackerAbilitiesModifier(u32 battlerAtk, uq4_12_t typeEffectivenessModifier, bool32 isCrit, u32 abilityAtk)
{
    ...
    case ABILITY_PRECISION:
        if (isCrit && gBattleMons[battlerAtk].statStages[STAT_ACC] > DEFAULT_STAT_STAGE)
            return UQ_4_12(1.0 + (0.1 * CountBattlerAccuracyIncreases(battlerAtk)));
        break;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities, uq4_12_t typeEffectivenessModifier)
{
    ...
    if ((moveType == defType) && gStatuses4[battlerDef] & STATUS4_REFLECTED_TYPE && typeEffectivenessModifier >= UQ_4_12(1.0))
        mod = UQ_4_12(0.5);
    else if ((moveType == defType) && gStatuses4[battlerDef] & STATUS4_REFLECTED_TYPE && typeEffectivenessModifier <= UQ_4_12(1.0))
        mod = UQ_4_12(0.25);
    if (moveType == TYPE_PSYCHIC && defType == TYPE_DARK && gStatuses3[battlerDef] & STATUS3_MIRACLE_EYED && mod == UQ_4_12(0.0))
        mod = UQ_4_12(1.0);
    if (gBattleMoves[move].effect == EFFECT_FREEZE_DRY && defType == TYPE_WATER)
        mod = UQ_4_12(1.5);
    if (gBattleMoves[move].effect == EFFECT_UPROOT_EVIL && (defType == TYPE_POISON || defType == TYPE_DARK))
        mod = UQ_4_12(1.5);
